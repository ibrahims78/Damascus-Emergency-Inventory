...
type MovementType = 'in' | 'out' | 'adjust' | 'custody_out' | 'custody_return' | 'damage' | 'central_return';
    ...
    <div className="space-y-6 print:space-y-3" dir="rtl">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
          ...
          {(equipment.maintenanceNotes || equipment.notes) && <Info label="الملاحظات" value={equipment.maintenanceNotes || equipment.notes} className="sm:col-span-2 lg:col-span-4" />}
        </CardContent>
      </Card>
      ...
      <Card className="print:shadow-none">
        <CardHeader className="pb-4 print:hidden">
          ...
          <div className="grid gap-3 pt-3 md:grid-cols-2"><div className="relative"><Search className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={document} onChange={(event) => setDocument(event.target.value)} placeholder="بحث برقم المستند" className="pr-9" /></div><Select value={type} onValueChange={(value) => setType(value as MovementType | 'all')}><SelectTrigger><SelectValue placeholder="كل الحركات" /></SelectTrigger><SelectContent><SelectItem value="all">كل الحركات</SelectItem>{Object.entries(MOVEMENT_META).map(([value, meta]) => <SelectItem key={value} value={value}>{meta.label}</SelectItem>)}</SelectContent></Select></div>
        </CardHeader>
        <CardContent className="p-0">
