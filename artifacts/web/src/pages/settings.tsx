...
function ImportEquipmentTab() {
  ...
  const handleExportTemplate = async () => {
    ...
    const dataHeaders = [
      ...
      'الحالة', 'سنة الصنع', 'بلد المنشأ', 'الحائز الحالي', 'ملاحظات',
    ];
    const dataWs = XLSX.utils.aoa_to_sheet([dataHeaders]);
    dataWs['!cols'] = [
      { wch: 30 }, { wch: 20 }, { wch: 20 }, { wch: 22 },
      { wch: 18 }, { wch: 14 }, { wch: 18 }, { wch: 22 }, { wch: 30 },
    ];
    ...
    const instrRows = [
      ...
      [],
      ['مثال على صف بيانات:'],
      ['جهاز قياس ضغط الدم الرقمي', 'جهاز طبي', 'BPM-2000', 'SN-2024-001', 'جيدة', '2022', 'ألمانيا', 'قسم الإسعاف', 'شاشة LCD'],
    ];
    const instrWs = XLSX.utils.aoa_to_sheet(instrRows);
    instrWs['!cols'] = [
